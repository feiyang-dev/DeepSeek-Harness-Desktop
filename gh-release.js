'use strict';
// 用法: node --use-system-ca gh-release.js
// 1) 创建 vX.Y.Z 的 GitHub Release（正文取 RELEASE_NOTES.md）
// 2) 上传 Windows 安装包到该 Release
// Token 从 Windows 凭据管理器读取（git:https://github.com）
...
async function uploadAsset(releaseId, filePath) {
  ...
  const url = `https://uploads.github.com/repos/${OWNER}/${REPO}/releases/${releaseId}/assets?name=${encodeURIComponent(baseName)}`;
  const r = await request(url, {
    method: 'POST',
    headers: {
      'User-Agent': 'dsh-desktop-release',
  ...
  if (r.status !== 201) {
    throw new Error(`上传 ${baseName} 失败 HTTP ${r.status}: ${r.body.slice(0, 500)}`);
  }
  const j = JSON.parse(r.body);
...
async function main() {
  ...
  const exe = `DeepSeek Harness 桌面版-Setup-${VERSION}.exe`;
  const exePath = path.join(__dirname, 'release', exe);
  if (!fs.existsSync(exePath)) {
    throw new Error(`安装包不存在: ${exePath}`);
  }
  await uploadAsset(releaseId, exePath);
  console.log('\n全部完成。release_id=' + releaseId);
}
